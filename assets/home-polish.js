(() => {
  ...
  function calendar(copy) {
    ...
    let index;
    try {
      const saved = JSON.parse(localStorage.getItem('yeye-daily-poster') || 'null');
      index = saved?.date === stamp ? saved.index : Math.floor(Math.random() * messages.length);
      localStorage.setItem('yeye-daily-poster', JSON.stringify({ date: stamp, index }));
    } catch {
    ...
    card.innerHTML = `<div class="daily-poster__top"><span>${today.toLocaleString('en', { month: 'long' }).toUpperCase()}</span><b>${today.getDate()}</b></div><div class="daily-poster__week">${today.toLocaleDateString('zh-CN', { weekday: 'long' })}</div><p class="daily-poster__message">${messages[index]}</p><small class="daily-poster__foot">DAILY NOTE · 每日一签</small>`;
    card.onclick = () => {
      index = Math.floor(Math.random() * messages.length);
      try { localStorage.setItem('yeye-daily-poster', JSON.stringify({ date: stamp, index })); } catch {}
      card.querySelector('.daily-poster__message').textContent = messages[index];
    };
    copy.append(card);
  }
  ...
  function polish() {
    ...
    if (wall && !wall.dataset.randomized) {
      ...
      const sources = shuffle([...new Set(imgs.map(img => img.currentSrc || img.src).filter(Boolean))]);
      if (sources.length > 1) imgs.forEach((img, i) => { img.src = sources[i % sources.length]; });
